...
class MyProblem(ea.Problem): # 继承Problem父类
    def __init__(self):
        ...
        #lb = [90, 7, 7,0.65]  # 决策变量下界
        lb = [90, 7, 7, 0.65]
        #ub = [120, 12, 10,0.75]  # 决策变量上界
        ub = [120, 12, 10, 0.75]
    ...
    def aimFunc(self, pop):  # 目标函数，pop为传入的种群对象
        ...
        x4 = Vars[:, [3]]
        #pop.ObjV = x1 * x2 * x3 * x4 * 1.15
        pop.ObjV = x1 * x2 * x3 * x4 * 1.15
        # 采用可行性法则处理约束，生成种群个体违反约束程度矩阵
        #pop.CV = np.hstack([,x1 * x2 * x3 * x4 * 1.15 - 8000])  # 第三个约束
        pop.CV = np.hstack([-10*(-1)*(-455.938353940571/ (x1 * x2 * x3 * x4 * 1.15)), -455.938353940571 /(x1 * x2 * x3 * x4 * 1.15) - 10, 0.5 - 0.45 * x3 + 0.1 *(x2 **2 / x3) * x4 - 0.42 * x2, 0.85 - x3 / x2, x3 /x2 - 0.98, x1* x2* x3* x4 * 1.15 - 8000])
